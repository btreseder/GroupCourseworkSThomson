...
T = zeros(tn/dt,length(z)); % ICs and Zeros Array for Temperature
...
for n = 1:(tn/dt)                          % Horizontal Position in Array
    ...
    Unum = find_U(B,tau,U_0);            % Upward Flux Numerical solution
    Dnum = find_D(B,tau,D_0);            % Downward Flux Numerical solution
    dNdp = finddNdp(Unum, Dnum, p(n,:)); % Net Flux differential
